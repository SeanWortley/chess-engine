...
fn main() {
    ...
    let baseline = EngineConfig::new(String::from("v6.0"), baseline_path, baseline_args);

    // Candidate config
    let mut candidate_path = PathBuf::new();
    candidate_path.push("target/release/engine");
    let candidate_args: Vec<String> = Vec::new();
    let candidate = EngineConfig::new(String::from("v7.0"), candidate_path, candidate_args);
